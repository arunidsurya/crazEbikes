...
async function handleCartView(req, res, next) {
  ...
  try {
    ...
    res.render("user/cart", {
      ...
      cartTotal,
      userId,
      totalQuantity: cartDataResult[0].totalQuantity,
      imgUri,
      images,
      formatPrice,
    });
...
async function handleAddToCart(req, res, next) {
  ...
  try {
    ...
    const existingCartItem = await Cart.findOne({
      userId: userId,
      cartItems: { $elemMatch: { product_id: product_id } },
    });
  ...
  try {
    ...
    for (const product_id of wishlistProductIds) {
      ...
      if (cartItemMap.has(product_id.toString())) {
        ...
        // If the product is not in the cart, add it
        cartItemMap.set(product_id.toString(), {
          product_id,
  ...
  try {
    ...
    if (products.length === 0) {
      return res.render("user/cart", {
        imgUri,
        images,
        message: "0 items in your wishlist",
      });
    }
...
async function handleEditAddress(req, res) {
  const { id, name, contactNumber, pincode, address, city, state, userId } =
    req.body;

  try {
    const user = await User.updateOne(
      { _id: userId, "delivery._id": id },
      {
        $set: {
          "delivery.$.name": name,
...
async function handleAddNewAddress(req, res) {
  const userId = req.session.userId;
  console.log(userId);
  const { name, contactNumber, pincode, address, city, state } = req.body;
...
async function handlePlaceOrder(req, res) {
  ...
  try {
    ...
    const user = await User.findById(userId);
    const address = [];
    const selectedAddressObj = user.delivery.find(
      (deliveryAddress) => deliveryAddress._id.equals(selectedAddressId) // Use the `equals` method to compare ObjectId
    );
                ...
                { _id: userId },
                {
                  ...
                  $push: {
                    "wallet.transactions": {
                      source: source,
                      method: "debit",
                      description: "Payment completd for the order",
                      transaction_amount: priceToPay,
                    },
  ...
  try {
    ...
    const ordersData = userOrders.map((order) => {
      return {
        ...
        Status: order.Status,
        OrderDate: `${order.Order_date.getFullYear()}-${
          order.Order_date.getMonth() + 1
        }-${order.Order_date.getDate()}`,
        TimeStamp: `${order.TimeStamp.getFullYear()}-${
          order.TimeStamp.getMonth() + 1
...
async function handleVerifyPayment(req, res) {
  ...
  if (hmac == razorpay_signature) {
    ...
    if (order.payment_method == "WALLET") {
        ...
        { _id: userId },
        {
          ...
          $push: {
            "wallet.transactions": {
              ...
              method: "debit",
              description: "Payment completd for the order",
              transaction_amount: updateWalletAmount,
            },
          }, // Push transaction details
        },
        { new: true }
      ...
      const updateStatus = await Orders.findByIdAndUpdate(
        { _id: orderId },
        {
          $set: {
            Status: "Placed",
      ...
      { _id: userId },
      {
        ...
        $push: {
          "wallet.transactions": {
            source: source,
            method: "credit",
            description: "Payment added to the wallet",
